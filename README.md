Демон для проекта "Часы Уизли". <br>Отслеживает изменения координат пользователей в FireStore и отправляет клиенту arduino пакет с id пользователя и id сегмента часов по постоянному сокетному соединению
